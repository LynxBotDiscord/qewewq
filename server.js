...
client.on("message", msg => {
  ...
  if (kufur.some(word => msg.content.toLowerCase().includes(word)) ) {
    if (!msg.member.hasPermission("ADMINISTRATOR")) {
      msg.delete()
       msg.reply("Bu sunucuda küfürler **RexSky** tarafından engellenmektedir! Küfür etmene izin vermeyeceğim!").then(message => message.delete(3000));
    }
}
    }
});
...
//Settings!
const yourID = "495929799882047530"; //Instructions on how to get this: https://redd.it/40zgse //Kendi İD'nizi Yazın
const setupCMD = "r!kayıt" //İstediğiniz Komut Yapabilirsiniz örn : !kayıtol
...
if (roles.length !== reactions.length) throw "Rol listesi ve emojiler aynı uzunlukta olamaz!";


function generateMessages(){
    var messages = [];
    messages.push(initialMessage);
    for (let role of roles) messages.push(`Kayıt Olmak İçin **"🔥"** Emojisine Tıkla!`); // Burayı Değiştirmeyin.
    return messages;
}
  ...
  db.fetch(`saas_${msg.guild.id}`).then(i => {
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 'sea' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'selam') {
                  return msg.channel.send('**Aleyküm Selam Hoşgeldin.**')
      }
    }
    else if (i == 'kapali') {
